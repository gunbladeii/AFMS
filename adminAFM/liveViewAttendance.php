...
$Recordset2 = $mysqli->query("SELECT attendance.id, attendance.nama, attendance.noIC, attendance.date AS date, attendance.stationCode, stationName.name AS stationName, attendance.time, attendance.timeOut AS totalDay , attendance.month, employeeData.employeeStatus FROM 
  ...
  INNER JOIN stationName ON attendance.stationCode = stationName.stationCode)
   ...
   WHERE attendance.timeOut IS NOT NULL AND employeeData.employeeStatus NOT LIKE 'dump' AND attendance.noIC = '$noIC' ORDER BY date ASC");
...
<?php if(!empty($row_Recordset2['stationCode'])) {?>
              <table id="example2" class="table table-hover table-responsive-xl">
                ...
                <td><?php if($row_Recordset2['timeOut'] NOT NULL || !empty($row_Recordset2['timeOut'])){echo $row_Recordset2['timeOut'];}else{echo '<span class="badge badge-danger">No Data</span>';}?></td>
