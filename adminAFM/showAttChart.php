...
$attend = $mysqli->query("SELECT attendance.date AS date, attendance.stationCode, stationName.name AS stationName, COUNT(attendance.date) AS totalDay ,(COUNT(attendance.date)/26)*100 AS percentAtt, attendance.month FROM attendance INNER JOIN stationName ON attendance.stationCode = stationName.stationCode GROUP BY attendance.stationCode, attendance.month, attendance.year ORDER BY attendance.month, attendance.year DESC");
