...
$station = $mysqli->query("SELECT employeeData.id, login.nama, login.role, login.username AS emel, login.noIC, employeeData.stationCode, employeeData.employeeStatus, stationName.name, login.password FROM ((employeeData 
    ...
    WHERE login.role NOT LIKE 'administrator' AND employeeData.employeeStatus LIKE 'dump'");
...
<?php if ($totalRows_station > 0) { ?>
	...
	<table id="example1" class="table table-hover table-responsive-xl">
                    ...
                    <tr style="text-align:left">
                      ...
                      <td><?php if($row_station['role'] =='rider'){echo '<h6>Rider</h6>';}elseif($row_station['role'] =='ss'){echo '<h6>Station Supervisor</h6>';}elseif($row_station['role'] =='dump'){echo '<h6>Delete</h6>';}else{echo 'Administrator';}?></td>
