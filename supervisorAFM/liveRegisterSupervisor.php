...
$station = $mysqli->query("SELECT employeeData.id,login.nama, login.role, login.username AS emel, login.noIC, login.password, employeeData.stationCode FROM login INNER JOIN employeeData ON employeeData.noIC = login.noIC WHERE employeeData.stationCode = '$stationName' AND login.role NOT LIKE 'dump' ORDER BY login.nama ASC");
